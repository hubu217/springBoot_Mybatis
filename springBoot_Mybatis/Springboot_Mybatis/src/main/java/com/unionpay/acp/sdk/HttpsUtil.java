...
/*	static {//商户如果通过代理访问需要的改动点
    ...
    public static Response send(String urlStr, byte[] data, Map<String, String> reqHeader, String requestMethod){
        ...
        if (!urlStr.startsWith("https://") && !urlStr.startsWith("http://")) {
        	...
        	if(idx <= 0) {
        		System.out.println("errurl [" + urlStr + "]");
        	}
            tag = urlStr.substring(0, idx); //取tag uhttps
            urlStr = "https" + urlStr.substring(idx); //地址转回https
		}
        
        if(data != null) {
        	System.out.println(requestMethod + " to [" + urlStr + "]: " + new String(data));
        } else {
        	System.out.println(requestMethod + " to [" + urlStr + "]");
        }
        ...
        try {
		            ...
		            for(Entry<String, String> kv : reqHeader.entrySet()){
			            	String keyHeader = kv.getKey();
			            	String valueHeader = kv.getValue();
			            	System.out.println("keyHeader="+keyHeader+"; valueHeader="+valueHeader);
			            	httpURLConnection.setRequestProperty(keyHeader,valueHeader);
		            ...
		            * if (tag != null) {
			                ...
			                if(ssLSocketFactoryMap.containsKey(tag)) {
				                	SSLSocketFactory sSLSocketFactory = ssLSocketFactoryMap.get(tag);
		...
		//            }
		            ...
		            if(httpCode != HttpURLConnection.HTTP_OK){
				                ...
				                resp.body = respBody;
				                if (respBody != null) {
				                	System.out.println("httpCode=" + httpCode + ", body: [" + new String(respBody, encoding) + "]");
				                } else {
				                	System.out.println("httpCode=" + httpCode + ", body is null");
				                }
		            }else{
				                respBody = read(httpURLConnection.getInputStream());
				                resp.body = respBody;
				                System.out.println("resp headers: " + resp.headers + ", body: [" + new String(respBody, encoding) + "]");
		            }
